...
public class MyWebSocketHandler {
    ...
    // only one game.
    private final Map<String, CrosswordGame> games = new HashMap<String, CrosswordGame>();
    ...
    private final Map<String, Integer> pointsTotalLastRoundMap = new HashMap<String, Integer>();

    public MyWebSocketHandler() {

        System.out
                .print("New connection detected! Instantiating new WebSocket, filling dictionary. ");
        try {
            knownWords = Helper.fillDictionary();
            System.out.println("There are " + knownWords.size()
                    + " words in the dictionary.");
        } catch (IOException e) {
            System.out.println("Exception filling dictionary: "
                    + e.getMessage());
            e.printStackTrace();
        }
    }
    ...
    public void onConnect(Session session) {
        ...
        System.out.println("New connection from " + clientIdentifier
        ...
        try {
            ...
            session.getRemote()
                    .sendString(
                            "Welcome, client! I started a new game for you and am ready to receive your words.");
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
    ...
    public void onMessage(Session session, String message) throws IOException {
        ...
        Integer pointsTotalLastRound = pointsTotalLastRoundMap
        ...
        session.getRemote().sendString(String.valueOf(pointsTotal));
    ...
    public void onClose(Session session, int statusCode, String reason) {
        System.out.println("Closed connection from "
                ...
                + session.getRemoteAddress().getPort() + " because of code "
                + statusCode + " (" + reason + ")");
    }
    ...
    public void onError(Session session, Throwable t) {
        System.out.println("Error with client "
                + session.getRemoteAddress().getAddress() + ":"
                + session.getRemoteAddress().getPort() + ": " + t.getMessage());
    }
    ...
    private String extractClientIdentifier(Session session) {
        String clientIdentifier = session.getRemoteAddress().getAddress() + ":"
